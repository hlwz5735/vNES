...
public class MapperDefault implements MemoryMapper {
    ...
    public void init(Nes nes) {
        ...
        this.cpuMemArray = cpuMem.getMem();
    ...
    public void write(int address, short value) {
        if (address < 0x2000) {
            // Mirroring of RAM:
            cpuMem.getMem()[address & 0x7FF] = value;
        } else if (address > 0x4017) {
            cpuMem.getMem()[address] = value;
...
//                }
    ...
    public void loadBatteryRam() {
        if (rom.isBatteryRam()) {
            ...
            if (ram != null && ram.length == 0x2000) {
                ...
                System.arraycopy(ram, 0, cpuMem.getMem(), 0x6000, 0x2000);
            }
        }
    }
    ...
    protected void loadRomBank(int bank, int address) {
        ...
        System.arraycopy(rom.getRomBank(bank), 0, cpuMem.getMem(), address, 16384);
    }
    ...
    protected void loadVromBank(int bank, int address) {
        ...
        System.arraycopy(rom.getVromBank(bank % rom.getVromBankCount()), 0,
                nes.getPpuMemory().getMem(), address, 4096);
